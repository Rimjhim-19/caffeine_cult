...
End-to-end training, rewritten against EDA findings. Pipeline:

  1. Load train sources + ground truth.
  2. Entity-level train/val split of Source1 (no pair leakage across folds).
  3. Blocking (country-partitioned, multi-channel -- see blocking.py) on
     the train fold -> candidate pairs + reusable channel similarity scores.
  4. Label each candidate pair from ground truth (hard negatives).
  5. Feature engineering, reusing blocking's channel scores (features.py).
  6. Train LightGBM.
  7. Repeat blocking+features on the validation fold (never seen in
     training), sweep the decision threshold, apply the global dedup
     assignment (assignment.py), and report the actual macro F_0.5 the
     challenge scores on.

Threshold sweep is biased toward the high end: EDA G5 recommends a
conservative threshold (~0.70-0.75) given F_0.5's precision weighting and
a 26-41% distractor rate in the candidate pool -- the sweep below still
checks the full range so this isn't hardcoded, just noted here as the
expected outcome.
...
from blocking import generate_all_candidates, measure_recall_ceiling
...
def split_source1(source1_df: pd.DataFrame, val_frac: float = 0.2, seed: int = 42):
    ...
    return (source1_df[train_mask].reset_index(drop=True),
            source1_df[~train_mask].reset_index(drop=True))
...
def build_feature_table(
    ...
    """
    Turn candidate pairs into a feature table. If ground_truth is given,
    adds a `label` column (1 = true match, 0 = hard negative) for
    training; omit it for inference on unlabeled test data.
    """
    ...
    for s1_id, cand_ids in candidates.items():
        ...
        for cand_id in cand_ids:
            ...
            feats = compute_pair_features(
                ...
                prefix,
                channel_scores=channel_scores.get((s1_id, cand_id)),
            )
...
def train_matcher(feature_df: pd.DataFrame) -> lgb.Booster:
    ...
    # NOTE: replace num_boost_round with early stopping against a proper
    # held-out eval_set once training on the full real dataset -- fixed
    # round counts are only a reasonable default for quick iteration.
...
def predict_matches(
    ...
    """
    Score candidate pairs, keep those above threshold, then apply the
    global dedup assignment (EDA B7: each S2/S3 id belongs to at most one
    S1 entity) so no candidate is double-claimed across S1 rows.
    """
    if feature_df.empty:
        return {}
    ...
    above_threshold = [(s1, c, sc) for s1, c, sc in triples if sc >= threshold]
    return deduplicate_assignment(above_threshold)
...
def tune_threshold(
    ...
    Full range swept (not hardcoded high) -- EDA G5 expects the optimum
    to land around 0.70-0.75 given F_0.5's precision weighting, but this
...
def main():
    ...
    parser.add_argument("--top-k", type=int, default=20,
                         help="per-channel, per-source candidate count "
                              "(EDA: K=20 union-channel reaches "
                              "98.46-100%% recall; K=10 already reaches "
                              "97.84-100%%)")
    parser.add_argument("--row-batch-size", type=int, default=2000,
                         help="S1 rows processed per blocking batch "
                              "(memory/speed tradeoff at multi-million scale)")
    ...
    train_feats = build_feature_table(s1_train, s2, s3, train_candidates,
                                       train_channel_scores, ground_truth=gt)
    print(f"[features] {len(train_feats)} labeled training pairs "
          f"({int(train_feats['label'].sum())} positive)")

    booster = train_matcher(train_feats)

    val_candidates, val_channel_scores = generate_all_candidates(
        s1_val, s2, s3, top_k=args.top_k, row_batch_size=args.row_batch_size)
    val_feats = build_feature_table(s1_val, s2, s3, val_candidates,
                                     val_channel_scores, ground_truth=None)
    ...
    with open(args.threshold_out, "w") as f:
        json.dump({"threshold": best_t, "val_f0.5": best_f,
                    "top_k": args.top_k, "row_batch_size": args.row_batch_size}, f, indent=2)
